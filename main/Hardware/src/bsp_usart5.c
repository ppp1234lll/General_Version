...
/*
*********************************************************************************************************
*    函 数 名: usart5_disable_dma
*    功能说明: 关闭DMA配置
*    形    参: 无
*    返 回 值: 无
*********************************************************************************************************
*/
void usart5_disable_dma(void)
{
#if defined(USE_USART5_IDEL) || defined(USE_USART5_TIMEOUT)
...
#ifdef USE_USART5_TX_DMA
    dma_channel_disable(USART5_DMAx, USART5_TX_DMA_CHANNEL);
    dma_flag_clear(USART5_DMAx, USART5_TX_DMA_CHANNEL, DMA_FLAG_FTF);
#endif
}
/*
*********************************************************************************************************
*    函 数 名: usart5_enable_dma
*    功能说明: 开启DMA配置
*    形    参: 无
*    返 回 值: 无
*********************************************************************************************************
*/
void usart5_enable_dma(void)
{
#if defined(USE_USART5_IDEL) || defined(USE_USART5_TIMEOUT)
    dma_channel_enable(USART5_DMAx, USART5_RX_DMA_CHANNEL);
#endif
...
{
    dma_memory_address_config(USART5_DMAx, USART5_RX_DMA_CHANNEL, DMA_MEMORY_0, (uint32_t)g_usart5_rx_buff);
...
{
    if (g_usart5_TransferState == TRANSFER_RX_COMPLETE)
    {
        g_usart5_TransferState = TRANSFER_WAIT;
        g_usart5_rx_buff[g_usart5_Len] = '\0';
        return g_usart5_rx_buff;
    }
    else
    {
        return NULL;
