...
typedef enum {
    SAVE_FLAG_DEVICE_PARAM   = (1 << 0),  /* 存储设备参数 */
    SAVE_FLAG_LOCAL_NETWORK  = (1 << 1),  /* 保存本地网络参数 */
    SAVE_FLAG_REMOTE_NETWORK = (1 << 2),  /* 保存远端网络参数 */
    SAVE_FLAG_UPDATE_ADDR    = (1 << 3),  /* 保存更新地址 */
    SAVE_FLAG_COM_PARAMETER  = (1 << 4),  /* 通信相关参数 */
    SAVE_FLAG_CAREMA         = (1 << 5),  /* 摄像头参数 */
    SAVE_FLAG_THRESHOLD      = (1 << 6),  /* 阈值 */
    SAVE_FLAG_RESET          = (1 << 7),  /* 恢复出厂化 */
    SAVE_FLAG_SNMP_OID       = (1 << 8),  /* 保存SNMP OID参数 */
    SAVE_FLAG_UPLOAD_ADDR    = (1 << 9),  /* 保存文件上传地址 */
    SAVE_FLAG_RS485_CONFIG   = (1 << 10), /* 保存RS485配置 */
} save_flag_e;

/* com_flag 通信标志位（按位使用，参照 save_flag.status） */
#define COM_FLAG_REPORT_NORMALLY_POS    0
#define COM_FLAG_REPORT_NORMALLY_MASK   0x03    /* 2 bits, 取值 0/1/2 */
#define COM_FLAG_QUERY_CONFIGURATION    (1 << 2)
#define COM_FLAG_HEART_PACK             (1 << 3)
#define COM_FLAG_VERSION                (1 << 4)
#define COM_FLAG_CONFIG_RETURN          (1 << 5)
#define COM_FLAG_IPC_INFO               (1 << 6)
#define COM_FLAG_SNMP_PARAM             (1 << 7)
#define COM_FLAG_PING_INFO              (1 << 8)
#define COM_FLAG_REPORT_IMMEDIATELY     (1 << 9)
...
void app_send_data_task_function(uint8_t *buf, uint16_t size);              /* 发送数据任务 */
...
void app_set_reply_src(uint8_t src);                             /* 记录指令来源通道(回复原路返回) */
...
void app_set_save_infor_function(uint32_t flags);                 /* 设置保存信息标志位 */
