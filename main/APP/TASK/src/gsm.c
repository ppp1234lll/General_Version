...
/* 统一回收所有已完成的无线(GSM)后台任务: 在 gsm 任务(外部)上下文删除, 立即释放栈/TCB,
 * 避免任务自删除延迟回收造成的堆积/碎片与重建失败 */
static void gsm_reap_stopped_tasks(void)
{
    update_gsm_delete();
    upload_gsm_delete();
}
...
__RESET:
    ...
    while(1)
    {
        gsm_reap_stopped_tasks();   // 同步回收已完成的无线后台任务(立即释放栈/TCB)
        gprs_sim_status_monitoring_function();
        gprs_csq_status_monitoring_function();
        /* 查看是否允许无线网络连接 */
        if( (app_get_network_mode() != SERVER_MODE_LWIP ))    
        {
            /* DATA 业务始终运行, OTA/Upload 不阻塞主链路:
             * 驱动层 g_gprs_at_mutex 递归锁串行化每条 AT 往返, rtcp URC 按 client_id 优先分发,
             * per-link disconn 独立标记, 三链路可在单 UART 上安全时分并行 */
            ...
            if(sg_gsmoperate_t.tcp_status == 1 && (sg_gsm_flag&0x8000))        /* 数据发送 */
            {
                ...
                send_ret = gprs_network_data_send_function(sg_gsm_buff,(sg_gsm_flag&0x7fff));
                sg_gsm_flag = 0;
                if(send_ret == GPRS_SEND_DISCONN)
                {
                    /* 550(TCP发送失败)通常是模块意外重启,程序仍以为连接正常;
                     * 断开链路、置 tcp_status=0, 由 gsm_tcp_control_function() 接管:
                     *   其内部有 CEREG 检测 → 网络异常则触发模块重启;
                     *               CEREG 正常则直接重连。 */
                    gprs_network_disconnect_function(GPRS_LINK_DATA);
                    sg_gsmoperate_t.tcp_status = 0;
                    app_set_send_result_function(SR_SEND_ERROR);
                }
                else if(send_ret != GPRS_SEND_OK)
                {
                    app_set_send_result_function(SR_SEND_ERROR);
                }
            }
...
void gsm_tcp_control_function(void)
{
    ...
    if(sg_gsmoperate_t.tcp_status == 0) 
    {
        ...
        else 
        {
            /* 连接主线路 */
            gprs_network_disconnect_function(GPRS_LINK_DATA);
            ret = gprs_network_connect_server((uint8_t *)remote->outside_iporname, remote->outside_port, GPRS_LINK_DATA);
