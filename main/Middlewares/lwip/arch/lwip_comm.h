...
/* "连接中(INIT_CONNECT)"状态看门狗超时, 单位为控制函数调用周期(20ms):
 * 任务仍在但长时间连不上时触发安全复位重连, 1500*20ms = 30s */
#define LWIP_TCP_INIT_CONNECT_TIMEOUT  (1500)
...
typedef struct  
{
    ...
    uint8_t resetting;         // 网络复位进行中，防止link线程干扰
}__lwip_dev;
