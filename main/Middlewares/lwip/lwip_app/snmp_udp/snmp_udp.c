...
int snmp_sock = -1;
static volatile uint8_t snmp_exit_req = 0;  /* 1: 任务已停到安全点, 待外部同步删除 */
        ...
        {    
            switch(sg_snmp_status_t.status)
            {
                    ...
                    switch(sg_snmp_status_t.snmp_dev_type)
                    {
                        case SNMP_IPC:
                            for(uint8_t i = 0;i < IPC_OID_MAX;i++)
                            {
                                ...
                                vTaskDelay(50);              // 发送间隔50ms, 接收统一在 SNMP_RECV 阶段处理
                            }
                            recv_times = 0; // 接收计时重置
                ...
                case SNMP_END:
                    ...
                    /* 不自删除, 也不在此复位 snmp_status: 置退出请求并挂起, 由 eth 任务
                     * 调用 snmp_delete() 在其它任务上下文同步删除并复位状态, 立即回收栈/TCB,
                     * 避免自删除延迟回收 + 重建竞态 */
                    snmp_exit_req = 1;
                    for(;;)
                    {
...
{
    BaseType_t res;
    if(snmp_exit_req){ return 0; }                  /* 有任务待回收, 本轮先不创建 */
    if(snmp_task_handle != NULL){ return pdPASS; }  /* 已存在, 不重复创建 */

    res = xTaskCreate(  (TaskFunction_t )snmp_thread,
...
*********************************************************************************************************
*    函 数 名: snmp_delete
*    功能说明: 回收已完成并停到安全点的SNMP任务; 须由 eth 任务等其它任务上下文周期调用,
*              在外部上下文 vTaskDelete 并复位状态, 立即回收栈/TCB, 避免自删除延迟回收
*    形    参: 无
*    返 回 值: 无
*********************************************************************************************************
*/
void snmp_delete(void)
{
    TaskHandle_t h;

    if(snmp_exit_req == 0){ return; }
