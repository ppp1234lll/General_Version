...
// RTC_Get_Time(&time);        // 获取到的时间没有年月日？
    snprintf(datetime,CREATED_LENGTH,"%04d-%02d-%02dT%02d:%02d:%02d.000Z",time->year,time->month,time->data,(time->hour-8),time->min,time->sec);
...
{    
    ...
    base64_encode((uint8_t*)sha1_encode, pwd_digest,sizeof(sha1_encode));
}
