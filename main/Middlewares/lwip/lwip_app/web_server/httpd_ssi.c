...
static void snmp_parameter_handler(char *pcInsert,uint8_t num)
{
    ...
    switch(num)
    {
        case 0:
            ...
            break;
        case 1:
            sprintf(pcInsert,"%d.%d.%d.%d",param->onv_ip[0],param->onv_ip[1],param->onv_ip[2],param->onv_ip[3]);
            break;
        default:
            sprintf(pcInsert,"");
            break;
...
/*
*********************************************************************************************************
*    函 数 名: httpd_ssi_carema_setting_function
*    功能说明: 摄像头
*    形    参: 
*    返 回 值: 
*********************************************************************************************************
*/
void httpd_ssi_carema_setting_function(char *pcInsert)
{
    char buff[6][20] = {0};
    char brand[6][5] = {0};
    char buff2[3]    = {0};
...
/*
*********************************************************************************************************
*    函 数 名: httpd_ssi_snmp_oid_function
*    功能说明: SNMP OID 配置 - 返回所有设备类型的 OID 数组
*    形    参: 
*    返 回 值: 
*********************************************************************************************************
*/
void httpd_ssi_snmp_oid_function(char *pcInsert)
{
    ...
    for (type = 0; type < 3; type++) {
        for (cmd = 0; cmd < IPC_OID_MAX; cmd++) {
            if (type != 0 || cmd != 0)
                p += sprintf(p, ",");
            p += sprintf(p, "\"%s\"", param->ipc_oid[type][cmd]);
        }
    }
    
    /* 光猫 (ONV) */
    for (cmd = 0; cmd < ONV_OID_MAX; cmd++) {
        p += sprintf(p, ",\"%s\"", param->onv_oid[0][cmd]);
    }
    
    /* 交换机 */
    for (cmd = 0; cmd < SW_OID_MAX; cmd++) {
        p += sprintf(p, ",\"%s\"", param->switch_oid[0][cmd]);
    }
