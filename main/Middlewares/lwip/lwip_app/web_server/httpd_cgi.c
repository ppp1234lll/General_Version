...
/*
*********************************************************************************************************
*    函 数 名: setting_camera_user_function
*    功能说明: 设置相机账号
*    形    参: 
*    返 回 值: 
*********************************************************************************************************
...
/*
*********************************************************************************************************
*    函 数 名: httpd_cgi_set_camera_account_function
*    功能说明: 设置相机账号
*    形    参: 
*    返 回 值: 
*********************************************************************************************************
...
static int8_t setting_snmp_function(char *pcParam[], char *pcValue[],uint8_t i)
{
    ...
    /* 解析参数名格式: s{type}_{cmd}，例如 s0_0, s1_5, s3_2, s4_4 */
    ...
    if (snmp_type <= 2)  /* IPC: 海康(0), 大华(1), 宇视(2) */
    {
...
int8_t httpd_cgi_update_function(int iNumParams, char *pcParam[], char *pcValue[])
{
    ...
    if (strcmp(pcValue[0] , "update_ota")==0) 
    {
        ...
        {
            /* 自动选择可用网口: 优先有线 */
            if(eth_get_tcp_status() == 2)  // 有线在线
            {
                update_set_update_mode(UPDATE_MODE_LWIP);
                set_return_status_function(0,(uint8_t*)"\"SUCCESS!\"");
            }
            else if(gsm_get_network_connect_status_function() == 1)  // 无线在线
            {
                update_set_update_mode(UPDATE_MODE_GPRS);
                set_return_status_function(0,(uint8_t*)"\"SUCCESS!\"");
            }
...
int8_t httpd_cgi_show_function(char *pcValue[], uint16_t *data, uint8_t *buff)
{
    ...
    if ( strcmp(pcValue[0] , "caremaSetting")==0 ) {    /* 摄像头IP */
        *data = 0;
        httpd_ssi_carema_setting_function((char*)buff);
        return 0;
    }

    if ( strcmp(pcValue[0] , "carema_user")==0 ) {    /* 摄像头账号 */
        *data = 0;
        httpd_ssi_carema_user_function((char*)buff);
        return 0;
    }

    if ( strcmp(pcValue[0] , "snmp_set")==0 ) {    /* SNMP OID 配置 */
        *data = 0;
