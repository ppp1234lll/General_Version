...
void ebtn_APP_Keys_Init(void)
{
    ...
    for (uint8_t key_id = 0; key_id < KEYS_COUNT; key_id++)
    {
        ...
        {
            if (special_keys_list[i].key_id == key_id && special_keys_list[i].params != NULL)
            {
    ...
    for (uint8_t i = 0; i < COMBO_KEYS_COUNT; i++)
    {
        ...
        {
            if (special_combo_keys_list[j].combo_key_id == combo_id && special_combo_keys_list[j].params != NULL)
            {
